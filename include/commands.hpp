...
struct P_work
    : public Instruction  // (PP)
{
    void execute(Memory &memory) override;
};

struct P_prog
    : public Instruction  // (PП)
{
    void execute(Memory &memory) override;
};
...
struct Return_clean
    : public Instruction  // (В/О)
{
    void execute(Memory &memory) override;
};

struct Stop_run
    : public Instruction  // (С/П)
{
    void execute(Memory &memory) override;
};

struct Jmp
    : public Instruction  // (БП)
{
    void execute(Memory &memory) override;
};

struct Subprog_step_run_in_work_mode
    : public Instruction  // (ПП/Т)
{
    void execute(Memory &memory) override;
};

struct Insert_exp
    : public Instruction  // (ВП)
{
    void execute(Memory &memory) override;
};
