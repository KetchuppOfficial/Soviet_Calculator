...
void Memory::mem_print(Memory& memory) { 
	for (int i = 0; i < 16; i++)
		std::cout << memory.regs_[i] << " "; 
	std::cout<<"\n";	
	}
void Memory::drum_print(Memory& memory) {
	for (int i = 0; i < 7; i++)
			std::cout << memory.regs_[i] << " ";	
	std::cout <<"\n";
}
...
Memory::arifmetic_type Memory::get_x() { return x_reg_; }

void Memory::set_y (arifmetic_type val) { y_reg_ = val; }
Memory::arifmetic_type Memory::get_y() { return y_reg_; }

void Memory::set_P_flag(bool val) { P_flag = val; }
bool Memory::get_P_flag() { return P_flag; }

void Memory::set_F_flag(bool val) { F_flag = val; }
bool Memory::get_F_flag() { return F_flag; }
